class RedirectMiddleware
  ...
  def call(env)
    ...
    Rails.logger.info " 1st id => #{request.host.match(/#{ENV['ADMIN_URL']}/ ) && !request.host.match(/#{ENV['ADMIN_URL']}\/admin.*/)}"
    Rails.logger.info " 2nd id => #{!request.host.match /#{ENV['ADMIN_URL']}/ && request.host.end_with?('/admin')}"
    Rails.logger.info "======================================"
